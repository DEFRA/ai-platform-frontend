...
Feature/technical specifications live under [.github/specs/](./specs/README.md) — check there before large changes for existing design docs.

### Using the shared MVP portal spec (frontend scope only)

[.github/specs/mvp-portal-ui-api-scope.md](./specs/mvp-portal-ui-api-scope.md) is a copy of a cross-repo specification shared with the `ai-platform-backend-api` repo — it documents the frontend, the backend and the overall architecture together, and is kept as a faithful, unannotated copy of the source document. **This repo is the frontend service (`ai-platform-frontend`) only** — when using this spec to plan or implement work here:

- Apply only the frontend-relevant sections: `Frontend structure`, the "Frontend" column in `UI and API orchestration` → `Responsibilities`, `Page inventory` (routes/screens/design tokens), the frontend rows in `Configuration`, the frontend rows/columns in `Delivery phases` and `Testing strategy`, and the frontend-facing parts of `Cross-cutting requirements` (CSP, CSRF, sessions, accessibility, logging — no secrets/PII).
- Treat `API v1 contract` as an external contract to call via the backend's `apiClient` — implement the HTTP calls and response/error handling, but do not implement the API server itself.
- Do **not** implement anything under `Backend structure`, `Domain model and MongoDB collections`, Azure/APIM adapter details, or backend-only environment variables (e.g. `MONGO_URI`, `AZURE_*`, `APIM_*`, `MAINTENANCE_TOKEN`) — those belong exclusively to the backend repo, which keeps its own copy of this spec scoped to its side.
- User journeys (J1–J4) and the architecture diagrams describe both services — read them for context on what the frontend sends/receives, but only build the `FE`/browser-facing steps.
- If a task needs backend behaviour that doesn't exist yet, note it as a dependency instead of implementing it here.
